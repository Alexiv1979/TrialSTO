...
Процедура ПриОткрытии(Отказ)    
	...
	Элементы.ДекорацияВидПроводимогоОсмотра.Заголовок = Строка(ВидПроводимогоОсмотра);
...
&НаСервере
Функция ЕстьОшибкиПриЗаполненииеквизитов() 
	
	Возврат СостояниеОсмотра <> Перечисления.ВидыСостоянийОсмотра.Ок 
		И (СокрЛП(Комментарий) = "" И СокрЛП(ИдентификаторыПрисоединенныхФайлов) = "");	
КонецФункции
...
Процедура ПриЗакрытии(ЗавершениеРаботы)    
	...
	// Удаляем индентификаторы, если фото удалены  
	ИдентификаторыПрисоединенныхФайловБезУдаленных(ОбъектОтбора, ИдентификаторыПрисоединенныхФайлов);	
	//ИдентификаторыПрисоединенныхФайлов = "";
	
	СтруктураДанных = Новый Структура("ФормаРедактирования, ИмяТаблицы, СостояниеОсмотра, Комментарий, ИдентификаторыПрисоединенныхФайлов", "ДобавлениеКомментария", ИмяТаблицы, СостояниеОсмотра, Комментарий, ИдентификаторыПрисоединенныхФайлов);
...
Процедура НажатиеОК(Команда)                         
	УстановитьСостояниеОсмотра("Ок");     
	ЭтаФорма.Закрыть();
КонецПроцедуры     


&НаКлиенте
Процедура НажатиеУдов(Команда)           
	УстановитьСостояниеОсмотра("Удов");       
КонецПроцедуры

&НаКлиенте
Процедура НажатиеКрит(Команда)
	УстановитьСостояниеОсмотра("Крит"); 
КонецПроцедуры    

&НаКлиенте
Процедура УстановитьСостояниеОсмотра(ВыбСостояние)
	
	СостояниеОсмотра = ПолучитьСостояние(ВыбСостояние);    
...
Процедура УстановитьЦветФонаКнопок()   
	...
	Элементы.КнопкаОК.ЦветФона   = ПолучитьЦветФонаКнопки("Ок");  
	Элементы.КнопкаУдов.ЦветФона   = ПолучитьЦветФонаКнопки("Удов");
	Элементы.КнопкаКрит.ЦветФона   = ПолучитьЦветФонаКнопки("Крит");
...
Функция ПолучитьЦветФонаКнопки(ВыбСостояние)         
	...
	Если СостояниеОсмотра = Перечисления.ВидыСостоянийОсмотра[ВыбСостояние] Тогда 		
...
Функция ПолучитьСостояние(ВыбСостояние)         
	...
	Если ВыбСостояние = "Ок" Тогда
		Возврат Перечисления.ВидыСостоянийОсмотра.Ок;
	ИначеЕсли ВыбСостояние = "Удов" Тогда
		Возврат Перечисления.ВидыСостоянийОсмотра.Удов;	
	ИначеЕсли ВыбСостояние = "Крит" Тогда
		Возврат Перечисления.ВидыСостоянийОсмотра.Крит;	
	КонецЕсли;          
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если ЕстьОшибкиПриЗаполненииеквизитов() Тогда
		Сообщение = Новый СообщениеПользователю;
		ТекстПредупреждения= "Не заполнен комментарий или не добавлен файл";    
